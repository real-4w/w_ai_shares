...
class StockApp:
    def __init__(self, root):
        ...
        # Setup logging
        logging.basicConfig(filename='stock_app.log', level=logging.ERROR, 
        ...
        self.ticker_label = tk.Label(self.ticker_frame, text="", font=("Courier", 12), bg="black", fg="yellow", anchor="w")
        ...
        # Start ticker tape in a separate thread
        self.ticker_thread = threading.Thread(target=self.update_ticker_tape, daemon=True)
    ...
    def add_ticker(self):
        ...
        if ticker and ticker not in self.ticker_stocks:
        ...
        elif ticker in self.ticker_stocks:
        ...
        while self.running:
            ...
            for ticker in self.ticker_stocks:
                try:
                    ...
                    color = "green" if change_percent != 'N/A' and change_percent > 0 else "red" if change_percent != 'N/A' else "yellow"
                    ticker_text += f"{ticker}: ${price:.2f} ({change_percent:+.2f}%)  |  "
                except Exception as e:
                    self.ticker_prices[ticker] = ('N/A', 'N/A')
                    ticker_text += f"{ticker}: N/A  |  "
                    logging.error(f"Ticker tape error for {ticker}: {e}")
            
            # Update ticker label with scrolling effect
            self.ticker_label.config(text=ticker_text)
            for i in range(len(ticker_text) + 50):
                if not self.running:
                    break
                self.ticker_label.config(text=ticker_text[i:] + ticker_text[:i])
                self.root.update()
                time.sleep(0.1)
            time.sleep(60)
    ...
    def fetch_stock_data(self, ticker):
        ...
        try:
            ...
            stock_data = {
                ...
                'Analyst Ratings': recommendations[['Strong Buy', 'Buy', 'Hold', 'Sell', 'Strong Sell']].tail(1) if not recommendations.empty else None
            }
    ...
    def display_daily_recommendations(self):
        ...
        for ticker in candidate_tickers:
            stock_data = self.fetch_stock_data(ticker)
            if 'Error' not in stock_data and stock_data['Analyst Ratings'] is not None:
                ratings = stock_data['Analyst Ratings']
                score = ratings.get('Strong Buy', 0) * 2 + ratings.get('Buy', 0) * 1 - ratings.get('Sell', 0) * 1 - ratings.get('Strong Sell', 0) * 2
